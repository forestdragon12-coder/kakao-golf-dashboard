...
if [ "$ACTIVE" = "true" ]; then
    log "SKIP: 사용 중 (${REASON}). 절전 취소."
    exit 0
fi

log "PROCEED: 미사용 판단 → 절전 프로세스 시작"
...
if [ -n "$CONSOLE_USER" ] && [ "$CONSOLE_USER" != "root" ]; then
    sudo -u "$CONSOLE_USER" osascript -e '
        display dialog "⚠️ 5분 후 Mac이 자동으로 절전 모드로 전환됩니다." & return & return & "작업 중이면 [취소]를 눌러주세요." buttons {"절전 진행", "취소"} default button "절전 진행" with title "⏰ 자동 절전 예고" with icon caution giving up after 290
    ' 2>/dev/null
    RESULT=$?

    if [ $RESULT -ne 0 ]; then
        log "CANCEL: 사용자가 절전을 취소함"
        exit 0
    fi
fi
...
if [ "$IDLE_FINAL" -lt 120 ]; then
    log "CANCEL: 대기 중 사용자 활동 감지. 절전 취소."
    if [ -n "$CONSOLE_USER" ] && [ "$CONSOLE_USER" != "root" ]; then
        sudo -u "$CONSOLE_USER" osascript -e '
            display notification "사용자 활동 감지로 자동 절전이 취소되었습니다." with title "자동 절전 취소" sound name "Glass"
        ' 2>/dev/null
    fi
    exit 0
fi
